...
export const HairstyleSelector: React.FC = () => {
  ...
  return (
    <div 
      className={`
        w-full glass-panel border-t border-white/10 shadow-[0_-10px_40px_rgba(0,0,0,0.8)]
        transition-all duration-500 cubic-bezier(0.32, 0.72, 0, 1) pb-8
    ...
    >
      ...
      <div className="px-6 mb-4 flex items-center justify-between cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
        ...
        <button className="glass-button p-2 rounded-full text-white/50 hover:text-white transition-colors active:scale-95 min-w-[40px] min-h-[40px] flex items-center justify-center">
          {isExpanded ? <ChevronDown size={20} /> : <ChevronUp size={20} />}
        </button>
      </div>
      
      {/* Content Area */}
      <div className={`transition-all duration-500 ease-in-out overflow-hidden ${isExpanded ? 'max-h-[340px] opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="flex gap-4 sm:gap-5 overflow-x-auto pb-6 pt-4 scrollbar-hide snap-x snap-mandatory px-6 -mx-6 pl-6">
            ...
            <button
              ...
              className={`
                ...
                ${selectedHairstyleId === style.id 
                  ? 'ring-2 ring-cyan-400 shadow-[0_0_30px_rgba(34,211,238,0.3)] scale-105 -translate-y-2' 
            ...
            >
              ...
              <div className={`absolute inset-0 bg-gradient-to-br transition-all duration-500 ${selectedHairstyleId === style.id ? 'from-gray-800 via-gray-850 to-gray-900' : 'from-gray-900 to-black'}`}>
                 <div className="absolute inset-0 flex items-center justify-center opacity-30 group-hover:opacity-50 group-active:opacity-60 transition-opacity">
                    <span className="text-6xl filter drop-shadow-lg grayscale group-hover:grayscale-0 transition-all duration-300">💇‍♀️</span>
                 </div>
      ...
      {!isExpanded && selectedStyleName && (
        <div className="px-6 animate-in fade-in slide-in-from-bottom-4 duration-300 pb-4">
          <div className="flex items-center justify-between glass-panel rounded-xl p-4 border border-white/10">
